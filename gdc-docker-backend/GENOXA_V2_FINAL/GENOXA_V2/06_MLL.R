...
roc_plot <- function(probability_list,
  ...
  # Sanity check: pROC::roc() requires BOTH classes present in the test set.
  # This happens when one tissue_type (tumor/normal) has too few samples for
  # the train/test split to place any of them into the test fold. Rather than
  # crashing the whole plot, skip that model and warn so the other models
  # (and the rest of the app) still render.
  test_group <- split_df$test$group
  if (nlevels(droplevels(test_group)) < 2) {
    warning(paste0(
      "roc_plot: test set contains only one class ('",
      paste(unique(as.character(test_group)), collapse = "', '"),
      "'). Cannot compute ROC/AUC. This almost always means one tissue_type ",
      "group (tumor/normal) has too few samples in the cohort - increase the ",
      "sample size for the minority group and re-run."
    ))
    return(NULL)
  }

  # Generate ROC objects and add them to a list, skipping any individual
  # model that fails (e.g. degenerate predicted-probability vector) instead
  # of aborting the entire plot.
  roc_list <- lapply(
    probability_list,
    function(x) {
      tryCatch(
        ...
        error = function(e) {
          warning(paste0("roc_plot: skipping a model - ", conditionMessage(e)))
          NULL
        }
      )
    }
  ...
  if (length(roc_list) == 0) {
    warning("roc_plot: no models produced a valid ROC curve.")
    return(NULL)
  }
