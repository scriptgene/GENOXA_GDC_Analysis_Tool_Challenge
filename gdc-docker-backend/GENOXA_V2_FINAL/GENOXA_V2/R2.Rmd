...
For gene modeling using ML models, we utilized the `caret (v. 6.0-94)` (Kuhn, Max, 2008) R package. Machine learning models are built using the differentially expressed genes (DEGs) identified from the analysis. The `voom-normalized expression values` of these genes are used as input features to train predictive models for classifying samples according to their experimental conditions. For data splitting, we set the size of the training dataset to `r params$gms1` and used `r params$gms2` as the random seed. For model training, we specified `r params$gmgmt1` as the resampling method, `r params$gmgmt3` as the number of resampling iterations, and `r params$gmgmt4` as the random seed. The machine learning algorithm chosen for predicting genes were `r params$gmgmt5`.
...
[5] R: A language and environment for statistical computing. R Core Team Technical Report R Foundation for Statistical Computing, Vienna, Austria, 2024. <https://www.R-project.org/>

[6] Chang W, Cheng J, Allaire JJ, Sievert C, Schloerke B, Xie Y, Allen J, McPherson J, Dipert A, Borges B (2024). shiny: Web Application Framework for R. <https://CRAN.R-project.org/package=shiny>
